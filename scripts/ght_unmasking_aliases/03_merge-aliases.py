...
for ix, row in enumerate(reader):
    ...
    a = Alias(record_type, uid, login, name, email, location, user_type, full=row[0])
    aliases[uid] = a
...
for uid, member_uids in clusters.iteritems():
    ...
    if is_valid:
        ...
        w_log.writerow([rep.uid, safe(rep.login), safe(rep.name), safe(rep.email), rep.location])
        for a in members:
            if a.uid != rep.uid:
                try:
                    w_log.writerow([a.uid, safe(a.login), safe(a.name), safe(a.email), a.location])
                except Exception, e:
                    print e
